...
static const Dcm_control_dids_conf dcmControlDids[] = // service 0x31
    {
        {0x010202, {dcm_NULL, dcm_NULL, service0x31SubFunctionCRCValidate}},   // crc
        ...
        {0x010203, {dcm_NULL, dcm_NULL, service0x31SubFunctionUpdateProgram}}, // 开始升级                                        // 检查升级pre condition
        {0x01ff05, {dcm_NULL}}};
...
uint8_t can_service_entry(uint8_t *data)
{
    ...
    case 0: // 单帧
    {
        ...
        update_app_entry(uds_frame.reqData, uds_frame.reqDataLength);
...
{
    uint8_t ret_value = 0;
    uart_printf(system_log, "service 0x31 SubFunction Start Erase at ", data, 4);
...
static uint8_t service0x31SubFunctionEraseResult(uint8_t *data, uint16_t dataLen)
{
    ...
    uart_printf(system_log, "service 0x31 SubFunction Erase Result\r\n", NULL, 0);

    uint32_t readAddress = (data[0] << 24) + (data[1] << 16) + (data[2] << 8) + data[3];

    // 检查flash地址合法性 擦写时，flash_erase会做判断
    ret_value = check_flash_address(readAddress);

    // flash的真实地址
    readAddress &= 0x0000FFFF;

    // 页地址
    readAddress = readAddress - (readAddress % 1024);
    readAddress |= 0x08000000;

    // 读取flash
    uint32_t *temp = (uint32_t *)readAddress;
    ...
    if (ret_value == true)
    {
        ...
        if (i != (1024 / 4))
        {
            ...
            convert_uint32_t_to_uint8_t(*temp, data_to_uint8_t);
            uart_printf(error_log | system_log, "read flash not is 0xff at :", data_to_uint8_t, 4);
            nrc = GeneralReject;
        }
    }
    uds_response(nrc);
...
static uint8_t service0x31SubFunctionCRCValidate(uint8_t *data, uint16_t dataLen)
{
    ...
    uart_printf(system_log, "service0x31SubFunctionCRCValidate\r\n", NULL, 0);
    uint16_t i; // loop
    // 需要进行crc运算的数据

    // 取出传入的crc
    uint32_t inputCRC = 0x00;

    uint8_t nrc = ControlPositiveResponse;
    // 转换传入的数据

    // 将 ota  data的数据转换成32位后的长度
    ((ota_data.dataLength % 4) != 0) ? (i = 1) : (i = 0);
    uint32_t crc_data[(ota_data.dataLength / 4) + i];
    // 设置crc_data 为0,
    for (i = 0; i < sizeof(crc_data) / sizeof(uint32_t); i++)
    {
        crc_data[i] = 0x00;
    }

    // 将ota data 转换成uint32_t
    for (i = 0; i < ota_data.dataLength; i++)
    {
        // data 进位
        crc_data[i / 4] |= ota_data.data[i] << (3 - (i % 4)) * 8;
    }
...
static uint8_t service0x31SubFunctionUpdateProgram(uint8_t *data, uint16_t dataLen)
{
    uart_printf(system_log, "service0x31SubFunctionUpdateProgram\r\n", NULL, 0);
    return 0;
}
...
static uint8_t service0x34Trigger(uint8_t *data, uint16_t dataLen)
{
    ...
    */
    ...
    // 数据长度
    uint8_t memorySize[memorySizeLength];

    memcpy(memorySize, &data[2], memorySizeLength);

    // 写入数据地址
    uint8_t memoryAddress[memoryAddressLength];

    memcpy(memoryAddress, &data[2 + memorySizeLength], memoryAddressLength);

    uart_printf(info_log, "memorySize :", memorySize, memorySizeLength);
    uart_printf(info_log, "memoryAddress :", memoryAddress, memoryAddressLength);

    // 请求的数据长度
    ota_data.reqDataLen = (memorySize[0] << 24) + (memorySize[1] << 16) + (memorySize[2] << 8) + memorySize[3];

    // 请求写入的数据地址
    ota_data.reqFlashAddress = (memoryAddress[0] << 24) + (memoryAddress[1] << 16) + (memoryAddress[2] << 8) + memoryAddress[3];
    ...
    ota_data.reciveDataTimes = 0x00;
    {
        ...
        uds_frame.resData[2] = 0x50; // 36 服务一帧的最大字节
...
static uint8_t service0x36Trigger(uint8_t *data, uint16_t dataLen)
{
    uart_printf(info_log, "36 service trigger data is ", data, dataLen);
...
{
    // 传入的数据没有规定
    uart_printf(info_log, "37 service trigger data is ", data, dataLen);

    // 数据传输完成 检查数据是否完成 并返回正反馈的nrc

    // 检查传入的数据长度和请求的数据长度
    if (ota_data.dataLength != ota_data.reqDataLen)
    {
        uds_response(GeneralReject);
        return false;
    }
...
void can_service_response(uint8_t *data, uint16_t dataLength)
{
    ...
    uart_printf(system_log, "can_service_response ", data, dataLength);
    ...
    else
    {
        ...
        for (uint8_t i = 0; i < frame_count; i++)
        {
            ...
            uart_printf(info_log, "can frame", can_data, 8);
            frame_low_4bits++;
            if (frame_low_4bits > 0x0f)
            {
                frame_low_4bits = 0x00;
            }
        }
        // 是否还有不满7字节的一帧需要发送
